...
def runStrat(openPrices: np.array, fastMA: np.array, slowMA: np.array) -> np.array:
    ...
    boughtOn=[]
    soldOn=[]
    combined_df = pd.DataFrame(np.array([openPrices, fastMA, slowMA]).T, columns=['open', 'fast', 'slow'])
    combined_df = combined_df.dropna(axis=0, how='any')
    openPricesTemp = combined_df['open'].values
    fastMATemp = combined_df['fast'].values
    slowMATemp = combined_df['slow'].values

    splitArray = splitSeven(fastMATemp, slowMATemp)
    difference = fastMATemp - slowMATemp
    fuzsys, fe, rec = buildFuzzySystem(splitArray)

    res = []
    for diff in difference:
        mf, ext = getFuzzyOutput(fuzsys, fe, rec, diff)
        res.append(mf)

    # The logical criteria for if a ma crossover happens, both on the buy and
    # sell side
    maBuy = lambda day: (
        res[day] > 0.5 # Strong Buy
    )
    maSell = lambda day: (
        res[day] < -0.5 # Strong Sell
    )

    for day in range(2, len(openPricesTemp)):
        # print(day),print(res[day]), print(res[day-1]), print(maBuy(day)), print(maSell(day))
        if not holding and maBuy(day):
            boughtAt = openPricesTemp[day]
            # print(day, 'buying at', boughtAt)
            boughtOn.append(day)
            holding = True

        elif holding and maSell(day):
            tradeRes.append(openPricesTemp[day]/boughtAt - 1)
...
def main(strat_eval, fitness_type, verbose=False) -> dict:
    ...
    for gen in range(0, NUM_GENERATIONS):
        ...
        # add random mutatations to strats
...
if __name__ == '__main__':
    ...
    mean_strat, mean_fitness, mean_bestStrats = main(strat_eval='compounded', fitness_type='mean', verbose=True)
    max_strat, max_fitness, max_bestStrats = main(strat_eval='compounded', fitness_type='max', verbose=True)
    median_strat, median_fitness, median_bestStrats = main(strat_eval='compounded', fitness_type='median', verbose=True)
    min_strat, min_fitness, min_bestStrats = main(strat_eval='compounded', fitness_type='min', verbose=True)
